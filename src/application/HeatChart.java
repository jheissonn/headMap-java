...
public class HeatChart {
    ...
    //private ColorNumberStrong[] colorNumberStrong;
	
	ArrayList<ColorNumberStrong> colorNumberStrong = new ArrayList<ColorNumberStrong>();
	private boolean dinamicColor;
	...
	public HeatChart(double[][] zValues) {
		this(zValues, min(zValues), max(zValues), false,new ArrayList<ColorNumberStrong>());
	}
	public HeatChart(double[][] zValues,Boolean teste) {
		this(zValues, 0, 3 , teste, new ArrayList<ColorNumberStrong>());
	}
	public HeatChart(double[][] zValues,Boolean teste, ArrayList<ColorNumberStrong> colorNumberStrong) {
		this(zValues, 0, 3 , teste, colorNumberStrong);
	 ...
	 * the z-values.
	 */
	public HeatChart(double[][] zValues, double low, double high,Boolean teste ,ArrayList<ColorNumberStrong> colorNumberStrong) {
		this.zValues = zValues;
		this.lowValue = low;
		this.highValue = high;
		this.dinamicColor = teste;
		this.colorNumberStrong = colorNumberStrong;
	...
	private void drawHeatMap(Graphics2D chartGraphics, double[][] data) {
			...
			for (int y=0; y<noYCells; y++) {
				...
				if(!dinamicColor) {
				  heatMapGraphics.setColor(getCellColour(data[y][x], lowValue, highValue));
					...
					if(data[y][x] > 0 ) {	
					 heatMapGraphics.setColor(getColorSelecionado(data[y][x]));
					}
					else {
						heatMapGraphics.setColor(new Color(255, 255, 255));
					}
	    ...
	    /*Draw number of rect start*/
		
		if(dinamicColor) {
		  int _Width = heatMapTL.x + cellSize.width/2;
		  int _Height = heatMapTL.y + cellSize.height / 2;
		  for (int x=0; x <noXCells; x++) {			
		  	for (int y=0; y< noYCells; y++) {		
		  		
		  		if(data[y][x] > 0) {
		  		  chartGraphics.setFont(new Font("TimesRoman", Font.PLAIN, 20));
		  		  chartGraphics.setColor(axisLabelColour);
                    chartGraphics.drawString(getNumberSelecionado(data[y][x]), _Width-10,  _Height+8);
		  		}
		  		
		  		_Height	= cellSize.height + _Height;	
		  	}
		  	_Width	= cellSize.width + _Width;
		  	_Height = heatMapTL.y + (cellSize.height / 2);			
		  }
		}
		/*Draw number of rect finish*/
		
	}
	
	private Color getColorSelecionado(Double inte) {
		for(ColorNumberStrong i : colorNumberStrong) {
			if(i.getStrong().toString().equals(inte.toString())) {
				return i.getCor();
			}			
		}		
	...
	private String getNumberSelecionado(Double inte) {
		for(ColorNumberStrong i : colorNumberStrong) {
			if(i.getStrong().toString().equals(inte.toString())) {
				return i.getNumber();
			}			
